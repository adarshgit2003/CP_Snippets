...
ll mod_div(ll a, ll b, ll m) {a = a % m; b = b % m; return (mod_mul(a, modulo_inverse(b, m), m) + m) % m;}  //only for prime m
...
vector<int> g[100005];
    ...
    vector<int> countVisitedNodes(vector<int>& edges) {
        int n=edges.size();
        for (int i=0; i<n; i++) g[i].push_back(edges[i]);
        vector<int> ans(n);
        for (int i=0; i<n; i++){
            if (vis[i]==0){
                v.clear();
                ...
                if (vis[v.back()]==1){
                    ...
                    for (int j=k; j<v.size(); j++) ans[v[j]] = cnt;
                    for (int j=0; j<k; j++) ans[v[j]] = (int)v.size()-j-1;
                }
...
void solve1(){
    ...
    vector<int> ans = countVisitedNodes(v);
