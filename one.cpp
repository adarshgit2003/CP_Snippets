...
vector<ll> seg(4*N);
void build(ll index, ll start, ll end, vector<ll>& v){
    if (start==end){
        seg[index]=v[start];
        return;
    }
    ll mid=(start+end)/2;
    build(2*index+1, start, mid, v);
    build(2*index+2, mid+1, end, v);
...
ll query(ll index, ll start, ll end, ll l, ll r){
    ...
    ll mid=(start+end)/2;
    ll left, right, ans;
    left = query(2*index+1, start, mid, l, r);
